...
export const GameOne: React.FC = () => {
  ...
  const renderCrocEmojiSticker = (word: string) => {
    ...
    return (
      <div className="w-24 h-24 md:w-28 md:h-28 bg-white dark:bg-slate-950/80 border border-slate-200 dark:border-slate-800 rounded-3xl flex items-center justify-center shadow-lg relative overflow-hidden group">
  ...
  return (
    <div className="w-full max-w-4xl flex flex-col items-center gap-6 md:gap-8 p-4 z-10 select-none">
      ...
      <span className="text-xs md:text-sm font-semibold bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 px-5 py-2.5 rounded-xl uppercase tracking-wider font-display flex items-center gap-2 flex-shrink-0 shadow-sm">
        <Palette className="w-5 h-5 stroke-[2.5px] text-indigo-500 dark:text-indigo-400" />
        Крокодил ойыны 🎨
      </span>
      ...
      <motion.div
        ...
        className="w-full bg-white/90 dark:bg-slate-800/80 border border-slate-200 dark:border-slate-700/50 text-slate-900 dark:text-white rounded-3xl p-6 md:p-10 flex flex-col items-center justify-center shadow-xl relative overflow-hidden"
      >
        {/* Rules explanation */}
        ...
        {/* Word Display frame */}
        <div className="w-full max-w-md bg-slate-50 dark:bg-slate-950/60 border border-slate-200 dark:border-slate-850 rounded-2xl p-6 md:p-8 flex flex-col items-center gap-4 mb-6 shadow-inner min-h-[220px] justify-center relative">
          ...
          <AnimatePresence mode="wait">
            {isWordVisible ? (
              ...
              >
                {/* Custom Illustration */}
                <div className="w-28 h-28 md:w-36 md:h-36 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-850 rounded-xl flex items-center justify-center p-2 shadow-md">
                  {renderCrocEmojiSticker(crocWord)}
                </div>
                
                {/* Active word string */}
                <h2 className="text-3xl md:text-4xl font-extrabold font-display text-slate-900 dark:text-white tracking-wide text-center">
                  {crocWord}
                </h2>
              </motion.div>
            ) : (
              ...
              >
                ...
                <div className="text-xs md:text-sm font-bold font-display text-slate-400 dark:text-slate-500 uppercase tracking-wider text-center mt-2">
                  Сөз жасырылды
                </div>
              </motion.div>
            )}
        ...
        <div className="flex flex-col sm:flex-row items-center gap-4 w-full max-w-md">
          ...
          <button
            ...
            className={`w-full py-3.5 px-6 font-bold font-display text-sm uppercase tracking-wider rounded-xl border shadow-lg hover:scale-105 active:scale-95 transition-all flex items-center justify-center gap-2 ${
              isWordVisible 
                ? 'bg-white dark:bg-slate-950 border-slate-200 dark:border-slate-800 text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white hover:bg-slate-100 dark:hover:bg-slate-900' 
